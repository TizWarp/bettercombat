...
public class ConfigurationHandler
{
	...
	public static int elenaiDodgeMainHandFeatherCost = 2;
	public static int elenaiDodgeOffHandFeatherCost = 1;
	public static int featherCostDamageRatioMainHand = 3;
	public static int featherCostDamageRatioOffHand = 3;
	public static float player_damage_multiplier = 0.5f;
	/* ARROWS --------------------------------------------------------------------------------------------------------------------- */
	...
	private static String[] weaponList =
	{
		 ...
		 "[Weapon Substring=rapier_] [Sound Type=BLADE] [Animation=STAB] [Property=ONEHAND] [Sweep=0] [Additional Reach=0.0] [Knockback=0.0] [Crit Chance=0.1] [Additional Crit Damage=0.0] [Parry=true] [Potion Effect=CRIT,AFFLICT,immersivecombat:bleeding,1,100]",
	...
	private static void loadConfiguration()
	{
		...
		elenaiDodgeMainHandFeatherCost = config.getInt("Main Hand Feather Cost", COMPAT, 2, 0, 20, "Main Hand Attacks will drain this amount of feathers");
		elenaiDodgeOffHandFeatherCost = config.getInt("Off Hand Feather Cost", COMPAT, 1, 0, 20, "Off Hand Attacks will drain this amount of feathers");
		featherCostDamageRatioMainHand = config.getInt("Feather Cost/Damage Ratio", COMPAT, 3, 1, 100, "Weapon Damage Value is divided by this and multipied by the base cost for total feather cost");
		featherCostDamageRatioOffHand = config.getInt("Feather Cost/Damage Ratio Off Hand", COMPAT, 3, 1, 100, "Same as above but for off hand");
		player_damage_multiplier = config.getFloat("Damage multiplier for first aid", COMPAT, 1.0f,0.01f,100.0f, "");
		...
		weaponList = config.getStringList("Custom Weapon Tweaker", WEAPON, weaponList,
			...
			+ "[Potion Effect=CRIT,AFFLICT,immersivecombat:bleeding,1,100] -> Critical strike: Afflict Bleeding I for 5 seconds.\n"
			+ "[Potion Effect=CRIT,RECEIVE,immersivecombat:brutality,1,200] -> Critical strike: Receive Brutality I for 10 seconds.\n");
		...
		for ( String s : shieldList )
		{
			...
			catch ( Exception e )
			{
				throw new Exception(
					...
					+ "\n\nError:     Invalid immersivecombat.cfg entry"
					...
					+ "\n\nFix:       Remove the line listed for 'Custom Shield Tweaker' or reset your config by deleting immersivecombat.cfg"
		...
		for ( String s : axeList )
		{
			...
			catch ( Exception e )
			{
				throw new Exception(
					...
					+ "\n\nFix:       Remove the line listed for 'Custom Axe Tweaker' or reset your config by deleting immersivecombat.cfg"
		...
		for ( String s : swordList )
		{
			...
			catch ( Exception e )
			{
				throw new Exception(
					...
					+ "\n\nFix:       Remove the line listed for 'Custom Sword Tweaker' or reset your config by deleting immersivecombat.cfg"
		...
		for ( String s : weaponList )
		{
			...
			catch ( Exception e )
			{
				throw new Exception(
					...
					+ "\n\nFix:       There was an error for the line in the 'Custom Weapon Tweaker' config. Remove the line or reset your config by deleting immersivecombat.cfg"
		...
		for ( String s : bowList )
		{
			...
			catch ( Exception e )
			{
				throw new Exception(
					...
					+ "\n\nFix:       Remove the line listed in 'Custom Bow Tweaker' or reset your config by deleting immersivecombat.cfg"
