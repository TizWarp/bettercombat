...
//		}
	...
	public static class Handler implements IMessageHandler<PacketOnItemUse, IMessage>
	{
		...
		private static void handle( PacketOnItemUse message, MessageContext ctx )
		{
			...
			if ( message.mainhand )
			{
				result = player.getHeldItemMainhand().getItem().onItemUse(player, player.world, blockPos, EnumHand.MAIN_HAND, EnumFacing.fromAngle(message.sideHit), 0.0F, 0.0F, 0.0F);
...
//				}
			...
			{
				result = player.getHeldItemOffhand().getItem().onItemUse(player, player.world, blockPos, EnumHand.OFF_HAND, EnumFacing.fromAngle(message.sideHit), 0.0F, 0.0F, 0.0F);
