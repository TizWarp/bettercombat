...
//		        	GlStateManager.rotate(-this.mainhandSprintingTimer*2 ,1.0F,0.0F,0.0F); /* Chopping rotation */
				...
				else if ( ClientProxy.EHC_INSTANCE.betterCombatMainhand.getAnimation().equals(ConfigurationHandler.Animation.STAB) )
				{
    ...
    protected void positionMainWeapon()
    {
    	...
    	if ( ClientProxy.EHC_INSTANCE.betterCombatMainhand.getAnimation().equals(ConfigurationHandler.Animation.CHOP) )
    	{
...
//			this.offhandAnimationTicks = 0.0F;
	...
	private void positionOffhandWeapon()
	{
		...
		/* If the weapon is an axe, position it upwards */
    	if ( ClientProxy.EHC_INSTANCE.betterCombatOffhand.getAnimation().equals(ConfigurationHandler.Animation.CHOP) )
    	{
        ...
        /* If the weapon is a spear, rotate it accordingly */
    	else if ( ClientProxy.EHC_INSTANCE.betterCombatOffhand.getAnimation().equals(ConfigurationHandler.Animation.STAB) )
        {
...
//			GlStateManager.translate(xx/10.0F, yy/10.0F, zz/10.0F);
    ...
    private void renderMainWeapon()
    {
        ...
        {
            boolean flag = this.itemRenderer.shouldRenderItemIn3D(ClientProxy.EHC_INSTANCE.itemStackMainhand) && Block.getBlockFromItem(ClientProxy.EHC_INSTANCE.itemStackMainhand.getItem()).getRenderLayer() == BlockRenderLayer.TRANSLUCENT;
    ...
    private void renderOffWeapon()
    {
        ...
        {
            boolean flag = this.itemRenderer.shouldRenderItemIn3D(ClientProxy.EHC_INSTANCE.itemStackOffhand) && Block.getBlockFromItem(ClientProxy.EHC_INSTANCE.itemStackOffhand.getItem()).getRenderLayer() == BlockRenderLayer.TRANSLUCENT;
    ...
    public class GuiCrosshairsBC extends Gui
    {
    	...
    	public void renderAttackIndicator( float partTicks, ScaledResolution scaledRes )
    	{
    		...
    		// if ( gamesettings.thirdPersonView == 0 )
    		{
    			...
    			{
        			if ( ClientProxy.EHC_INSTANCE.betterCombatMainhand.getWeaponProperty() == ConfigurationHandler.WeaponProperty.TWOHAND || ClientProxy.EHC_INSTANCE.betterCombatOffhand.getWeaponProperty() == ConfigurationHandler.WeaponProperty.TWOHAND || ClientProxy.EHC_INSTANCE.betterCombatOffhand.getWeaponProperty() == ConfigurationHandler.WeaponProperty.MAINHAND )
        			{
