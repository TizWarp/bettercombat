...
import dev.tizwarp.immersivecombat.network.PacketBreakBlock;
import dev.tizwarp.immersivecombat.network.PacketFastEquip;
import dev.tizwarp.immersivecombat.network.PacketHandler;
import dev.tizwarp.immersivecombat.network.PacketMainhandAttack;
import dev.tizwarp.immersivecombat.network.PacketOffhandAttack;
import dev.tizwarp.immersivecombat.network.PacketOnItemUse;
import dev.tizwarp.immersivecombat.network.PacketParrying;
import dev.tizwarp.immersivecombat.network.PacketShieldBash;
import dev.tizwarp.immersivecombat.network.PacketStopActiveHand;
...
import dev.tizwarp.immersivecombat.util.Reflections;
import dev.tizwarp.immersivecombat.util.SoundHandler;
...
public class EventHandlersClient
{
	...
	public boolean overwriteLeftClick(boolean checkBlocks)
	{
		 ...
		 */
		if (!player.getActiveItemStack().isEmpty() && this.betterCombatMainhand.getWeaponProperty() != ConfigurationHandler.WeaponProperty.TWOHAND)
		{
...
/*
		if (Loader.isModLoaded("elanaidodge2") && ConfigurationHandler.elanaiDodgeCompat) {
			if (getFeatherLevel(this.mc.player) < ConfigurationHandler.elenaiDodgeMainHandFeatherCost) {
				return;
			}
		}
*/
	...
	public void offhandAttack()
	{
		...
		if (ConfigurationHandler.elanaiDodgeCompat && Loader.isModLoaded("elanaidodge2")) {
			if (getFeatherLevel(this.mc.player) < ConfigurationHandler.elenaiDodgeOffHandFeatherCost) {
				return;
			}
		}
	...
	public boolean overwriteRightClick()
	{
		...
		if ( mov.typeOfHit == RayTraceResult.Type.BLOCK )
		{
			...
			if ( !this.offhandMouseoverHasEntity() || (this.mc.player.world.isRemote && block.onBlockActivated(this.mc.player.world, pos, state, this.mc.player, EnumHand.OFF_HAND, this.mc.objectMouseOver.sideHit, this.mc.objectMouseOver.sideHit.getXOffset(), this.mc.objectMouseOver.sideHit.getYOffset(), this.mc.objectMouseOver.sideHit.getYOffset())) )
			{
...
//	}
	...
	public boolean checkItemstackChangedMainhand(boolean force)
	{
																																...
																																// this.mc.player.getHeldItemMainhand())
		{
			...
			if (ConfigurationHandler.isItemClassWhiteList(this.itemStackMainhand.getItem()))
			{
				...
				for (ConfigurationHandler.CustomWeapon weapon : ConfigurationHandler.weapons)
				{
	...
	public boolean isMainhandAttackReady()
	{
		...
		if (Loader.isModLoaded("elenaidodge2") & ConfigurationHandler.elanaiDodgeCompat) {

			double damage = Helpers.getMainhandAttackDamage(this.mc.player, this.mc.player.getHeldItemMainhand());
			...
			return this.mainhandCooldown <= 0 & (getFeatherLevel(this.mc.player) - getWeight(this.mc.player)) >= cost;
		}
	...
	public boolean isOffhandAttackReady()
	{
		...
		if (Loader.isModLoaded("elenaidodge2") & ConfigurationHandler.elanaiDodgeCompat) {
			...
			double damage = Helpers.getOffhandAttackDamage(this.mc.player, this.mc.player.getHeldItemOffhand(),this.mc.player.getHeldItemMainhand());
			int cost = (int) ((damage/ ConfigurationHandler.featherCostDamageRatioOffHand)* ConfigurationHandler.elenaiDodgeOffHandFeatherCost);

			return this.offhandCooldown <= 0 & (getFeatherLevel(this.mc.player) - getWeight(this.mc.player)) >= cost;
		}
	...
	public void itemTooltipEventHigh(ItemTooltipEvent event)
	{
		...
		if (ConfigurationHandler.isItemClassWhiteList(event.getItemStack().getItem()))
		{
			...
			for (ConfigurationHandler.CustomWeapon s : ConfigurationHandler.weapons)
			{
...
//	}
	...
	private void updateBetterCombatTooltipHigh(ConfigurationHandler.CustomWeapon s, ItemTooltipEvent event)
	{
		/*
		 * =============================================================================
		 */
...
//			}
	...
	public RayTraceResult getMouseOverExtended(EntityPlayerSP player, double reach, float sweepWidth) // TODO /summon ender_dragon ~ ~ ~
	{
		...
		{
			lookEyes = player.getPositionEyes(this.mc.getRenderPartialTicks()).add(0.0D, player.getRidingEntity().getMountedYOffset(), 0.0D);
		}
		else
		{
		...
		final Vec3d lookTarget = lookEyes.add(lookVec.x * reach, lookVec.y * reach, lookVec.z * reach);
